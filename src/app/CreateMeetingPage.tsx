...
export default function CreateMeetingPage() {
	...
	async function createMeeting() {
		...
		try {
			...
			const members: MemberRequest[] = memberIds.map(id => ({user_id: id, role: "call_member"}))
			.concat({user_id: id, role: "call_member"})
			.filter(
...
function DescriptionInput({ value, onChange }: DescriptionInputProps) {
	...
	return (
		<div className="space-y-2">
			...
			<label htmlFor="" className="flext items-center gap-1.5">
				<input
					type="checkbox"
					checked={active}
					onChange={(e) => {
		...
		<div className="space-y-2">
			...
			{active && (
				<label htmlFor="" className="block space-y-1">
					...
					<textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder="Enter Participant email addresses separated by commas"
						className="w-full rounded-md border border-gray-300 p-2">
	...
	return <div className="text-center flex flex-col items-center gap-3">
		<div className="flex items-center gap-3">
			<span>
				Invitation link: {" "}
				<Link
				href={meetingLink} className="font-medium">
					{meetingLink}
				</Link>
			</span>
		...
		<a href={getMailToLink(meetingLink, call.state.startedAt, call.state.custom.description)}
		target="_blank" 
